...
export const TimelineView: React.FC = () => {
  ...
  const getWorkWeek = (offset: number) => {
    ...
    const monday = today
      .startOf("week")
      .add(1, "day")
      .add(offset * 7, "day"); // startOf("week") is Sunday
      ...
      member.assignedTasks.forEach((task: ITask) => {
        ...
        while (true) {
          const overlap =
            rows[rowIndex] &&
            !(
              dayjs(visibleSpan[visibleSpan.length - 1]).isBefore(
                rows[rowIndex].start
  ...
  const taskMap: Record<
    string,
    Record<string, (typeof scheduledTasks)[0][]>
  > = {};
  ...
  return (
    <div className="container px-4 mx-auto">
      ...
      <Paper
        elevation={1}
        className="overflow-x-auto rounded-md border-gray-200"
      >
        {/* Header */}
        ...
        <div className="max-h-[70vh] overflow-y-auto">
          {members.map((member) => (
            <Box
              key={member._id}
              className="flex items-stretch hover:bg-gray-50 transition"
            >
              <Box
                className="min-w-[200px] p-4 bg-white"
                sx={{
                  backgroundColor: member.isOverloaded
                    ? "#ffe5e5"
                    : "transparent",
                }}
              >
              ...
              {workWeek.map((day) => {
                ...
                return (
                  <Box
                    key={day.key}
                    className="min-w-[120px] p-2 relative h-[60px]"
                  >
                    {dayTasks.map((task) =>
                      task.span[0] === day.key ? (
                        <Tooltip
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              backgroundColor:
                                priorityColorMap[task.priority] || "#999",
                              color: "white",
                              borderRadius: "4px",
                              padding: "2px 8px",
                              fontSize: "11px",
